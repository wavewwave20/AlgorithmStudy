...
public class Main {
    ...
    public static void main(String[] args) throws Exception{
        ...
        //N과M 12
        distinctCombinationWithRepetition(0,0, new int[M]);
    ...
    static void distinctCombinationWithRepetition(int cnt, int start, int[] result) {
        if(cnt == M) {
            if(set.add(Arrays.toString(result))) {
                for(int i = 0; i<M; i++) {
                    sb.append(result[i]).append(" ");
        ...
        for(int i = start; i<N; i++) {
            result[cnt] = arr[i];
            distinctCombinationWithRepetition(cnt+1, i, result);
        }
    }
